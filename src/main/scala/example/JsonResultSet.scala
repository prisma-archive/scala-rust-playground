...
import play.api.libs.json.{DefaultReads, JsArray, JsObject, JsString, Json, Reads}
...
case class JsonResultSet(rows: IndexedSeq[JsObject]) extends ResultSet with DefaultReads {
  ...
  private def readColumnAs[T](columnLabel: String)(implicit reads: Reads[T], default: DefaultValue[T]): T = {
    ...
    columnValue match {
      case Some(value) => value.validate[T].getOrElse(sys.error(s"received incompatible value $value"))
